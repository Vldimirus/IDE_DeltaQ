...
- **Текущее release framing:** Linux release candidate. Репозиторий уже можно показывать и передавать внешним Linux-пользователям, но этот claim не распространяется на Windows/macOS.
...
- **Обработчик библиотек** — Linux-first `v1` путь imported pack для библиотек с `C ABI` через парсинг AST в libclang; более широкие `C++` wrapper-сценарии пока требуют thin adapter
...
- **Шаблоны проектов** — 4 user-facing файловых стартовых шаблона и internal regression fixtures загружаются из `resources/templates/` и копируются в новый проект как готовое дерево исходников
...
1. **Создать проект** — Файл → Новый проект, выбрать один из 4 user-facing шаблонов: Console Hello World, Console Counter Until Q, Desktop Text Editor или Desktop UI Baseline
...
В обычном `New Project` wizard доступны 4 user-facing шаблона:
...
- **Desktop Text Editor** — рекомендуемый desktop starter: notes workspace с menu bar, tool bar, tab strip, document workflow и editable event hooks
- **Desktop UI Baseline** — advanced desktop option: более тонкий baseline для graph/UI/runtime contract work и дальнейшей ручной desktop-настройки

Два старых raw-SDL desktop fixture-а остаются в репозитории только для internal/test flows и скрыты из обычного wizard.
